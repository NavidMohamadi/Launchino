...
Because these categories have zero real elements, `data/public_weight_profile.json` (the default weight profile applied to scraped/ingested vacancies — see §8) weights `CAP`/`TASK` at `0` — a real correctness bug where a nonzero weight on a data-less category was being silently renormalized away rather than scored was found and fixed 2026-07-27; see §5 and §15's resolved-defect entry, and `PROJECT_NOTES.md` for the full writeup.
...
**Default weighting for scraped vacancies**: `data/public_weight_profile.json` (loaded by `src/public_weighting.py`) is the single `category_weights` profile applied to any scraped/ingested vacancy without company-validated weights — `weighting_mode='balanced_default'` until a company confirms its own. As of v2.0.1 it weights `CAP`/`TASK` at `0` (fixed 2026-07-27 — see §4, §5, §15) after confirming this default was previously the actual root cause of a silent-renormalization bug affecting 81 of 86 real vacancies.
